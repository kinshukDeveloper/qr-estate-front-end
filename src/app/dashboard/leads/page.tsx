...
function LeadCard({ lead, onStatusChange, onDelete, onEdit }: LeadCardProps) {
  ...
  // Computed client-side only to avoid SSR/IST timezone hydration mismatch
  ...
  return (
    <div className="bg-[#111C28] border border-[#1A2D40] p-4 hover:border-[#1A2D40] transition-all">
      ...
      <div className="flex items-start justify-between gap-2 mb-3">
        <div>
          <div className="text-sm font-bold text-white">{lead.name || 'Unknown'}</div>
      ...
      <div className="relative bg-[#111C28] border border-[#1A2D40] w-full max-w-md p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <Field label="Message / Enquiry">
            <textarea className="resize-none input" rows={3} value={form.message || ''} onChange={e => set('message', e.target.value)} />
          </Field>
          ...
          <Field label="Notes">
            <textarea className="resize-none input" rows={2} placeholder="Internal notes..." value={form.notes || ''} onChange={e => set('notes', e.target.value)} />
          </Field>
...
export default function LeadsPage() {
  ...
  const todayFollowUps = leads.filter(l =>
    l.follow_up_date && (typeof window !== 'undefined' ? new Date(l.follow_up_date).toDateString() === new Date().toDateString() : false)
  ).length;
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-wrap gap-2">
        <div className="relative flex-1 min-w-48">
          ...
          <input
            ...
            className="py-2 text-sm input pl-9"
          />
        </div>
      ...
      {isLoading ? (
        ...
        <div className="py-20 text-center">
          ...
          <div className="mb-1 font-bold text-white">No leads yet</div>
