...
export default function DashboardPage() {
  ...
  // Only fetch once Zustand has rehydrated AND we have a token.
  // Without this guard the request fires with no Authorization header → 401 loop.
  useEffect(() => {
  ...
  // Greeting must be client-only — server runs UTC, users are IST (+5:30).
  // Computing on both sides gives different strings → hydration mismatch.
  ...
  const QUICK_ACTIONS = [
    { label: 'Add Listing',  description: 'Create a new property listing',   href: '/dashboard/listings/new', icon: Home,   color: '#00D4C8' },
    { label: 'Generate QR',  description: 'Create a QR code for a listing',  href: '/dashboard/qr',           icon: QrCode, color: '#FFB830' },
  ];
        ...
        {STATS.map(({ label, value, max, icon: Icon, color, hint }) => (
          <div key={label} className="bg-[#111C28] border border-[#1A2D40] p-5">
            <div className="flex items-start justify-between mb-4">
              ...
              {max && <span className="text-[10px] text-[#4A6580] font-mono">/ {max}</span>}
            </div>
          ...
          <div className="space-y-3">
            {NEXT_STEPS.map(({ step, label, done, href }) => (
              <Link key={step} href={href} className="flex items-center gap-3 group">
                <div className={`w-6 h-6 flex items-center justify-center text-xs font-bold flex-shrink-0 border transition-colors ${
                  done
                ...
                {!done && <ArrowRight size={14} className="ml-auto text-[#1A2D40] group-hover:text-[#00D4C8] transition-colors" />}
              </Link>
            ))}
          </div>
        </div>
          ...
          <div className="space-y-3">
            {QUICK_ACTIONS.map(({ label, description, href, icon: Icon, color }) => (
              <Link
                key={label}
                href={href}
                className="flex items-center gap-4 bg-[#111C28] border border-[#1A2D40] p-4 group transition-all hover:-translate-y-0.5"
              >
                <div className="flex items-center justify-center flex-shrink-0 w-10 h-10" style={{ background: `${color}15` }}>
