...
interface Listing {
  ...
  status: string;
  view_count: number;
  /** Short code used in the URL — e.g. "V7xKp2Qm" */
  short_code: string;
  agent_name: string;
  agent_phone?: string;
  agent_rera?: string;
...
function formatPrice(price: number, type: string) {
  ...
  if (price >= 10_000_000) return { main: `₹${(price / 10_000_000).toFixed(2)}`, unit: `Crore${suffix}` };
  if (price >= 100_000)    return { main: `₹${(price / 100_000).toFixed(2)}`,    unit: `Lakh${suffix}` };
...
export function PropertyPageClient({
  ...
  // ── Short URL (backed by /q/:shortCode redirect on the API) ──────────────
  const shortUrl = `${
    process.env.NEXT_PUBLIC_SHORT_URL_BASE ?? ''
  }/${listing.short_code}`;

  function handleShare() {
    const shareUrl = typeof window !== 'undefined' ? window.location.href : shortUrl;
    ...
    <div className="min-h-screen bg-[#F5F2EE] font-sans" style={{ fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      ...
      <div className="max-w-xl mx-auto px-4 pb-32">
        ...
        <div className="bg-white -mx-4 mt-3 px-5 py-3 border-b border-[#EEE] flex items-center justify-between">
          <span className="text-[10px] text-[#999] uppercase tracking-widest">Property Code</span>
        ...
        <div className="flex items-center gap-4 mt-4 text-xs text-[#999]">
          ...
          {listing.qr_scans != null && <><span>·</span><span>{listing.qr_scans} QR scans</span></>}
          <span>·</span>
